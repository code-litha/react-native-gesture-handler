# REACT NATIVE GESTURE HANDLER & STYLING

## Table of Contents

- [React Native Gesture Handler](https://docs.swmansion.com/react-native-gesture-handler/)
- [Tap Gesture](https://docs.swmansion.com/react-native-gesture-handler/docs/api/gestures/tap-gesture)
- [Pinch Gesture](https://docs.swmansion.com/react-native-gesture-handler/docs/api/gestures/pinch-gesture#example)
- [Create Card](https://v1.tailwindcss.com/components/cards)

## References

- [tailwind-color](https://tailwindcss.com/docs/customizing-colors)
- [tailwin-card](https://v1.tailwindcss.com/components/cards)
- [Transform in React Native](https://reactnative.dev/docs/transforms)
- [rn-text](https://reactnative.dev/docs/text-style-props)
- [rn-transform](https://reactnative.dev/docs/transforms)
